    ...
    public static class SecurityHelper
    {
        ...
        public static string EncryptAES(string dataToEncrypt, string masterPassword, out string saltBase64, out string ivBase64)
        {
            ...
            using var keyDerivation =
                new Rfc2898DeriveBytes(masterPassword, salt, Iterations, HashAlgorithmName.SHA256);
            ...
            // IMPORTANT: CryptoStream + StreamWriter scope
            using (var cs = new CryptoStream(ms, encryptor, CryptoStreamMode.Write))
            using (var sw = new StreamWriter(cs))
            {
                sw.Write(dataToEncrypt);
            } // ← streams CLOSE here (final block written!)
